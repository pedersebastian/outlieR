...
outlier_mean_sd <- function(.data, var, threshold) {
  tbl <-
    dplyr::summarise(.data,
      ...
      "outlier_vec" = list(out_help(!!var, upper, lower)),
      "outlier_exist" = length(unique(outlier_vec[[1]])) != 1,
      "outlier_pct" = mean(outlier_vec[[1]], na.rm = TRUE),
      "n_outliers" = sum(outlier_vec[[1]], na.rm = TRUE),
...
outlier_t_test <- function(.data, var, conf_int) {
  ...
  if (tbl$uniques < 4) {
    cli::cli_abort(c(
      "x" = "The Variable {tbl$var} have less than 4 unique values, witch is not allowed. ",
