library(outlieR)
atr_ignore <- c("tbls", "old_df", "vecs", "na_action", "filter_res", "class", "control")
...
A <- filter_outlier(tibble::as_tibble(mtcars), V12, V13, control = control_filter_outlier(
  discrete_method = "n",
  n_vars = 2
))
A_raw_meth <-
...
B <- filter_outlier(tibble::as_tibble(mtcars), V12, V13, V14, V15, control = control_filter_outlier(
  discrete_method = "n",
  n_vars = 3,
  na_action = "omit"
))
...
B_raw_meth <-
  ...
  dplyr::filter(V12 != "Other", V13 != "Other", V14 != "Other", V15 != "Other") |>
  dplyr::mutate(
    V12 = droplevels(V12),
    V15 = droplevels(V15)
  )

C <- filter_outlier(tibble::as_tibble(mtcars), V12, V13, V14, V15, control = control_filter_outlier(
  discrete_method = "n",
  n_vars = 3,
  na_action = "keep"
))
